        ...
        <q-card flat bordered class="bg-white text-dark shadow-1">
          ...
          <q-card-section>
            ...
            <div class="text-body1 q-pl-lg">
              <div>
                1.
                <strong>저장형 XSS</strong>를 이용하여 <code style="color: red">view.php</code>에서
                <code style="color: red">{{ scripttag }}</code> 가 실행됨
              </div>
              <div class="q-py-sm">
                2. <strong>Selenium 관리자 봇</strong>은 미리 정의된 관리자 세션(
                <code style="color: red">ADMINID</code> )으로 해당 게시글을 열람
              </div>
              <div>
                3. 스크립트가 실행되며 관리자 쿠기가
                <code style="color: red">attacker_server.php</code>로 전송됨
              </div>
              <div class="q-py-sm">
                4. 사용자가 탈취한 세션 쿠키를
                <code style="color: red">claim_admin_session.php</code> 에 제출하면
                <code style="color: red">$_COOKIE</code>에 주입<br />되어 관리자처럼 동작 가능
              </div>
              <div>
